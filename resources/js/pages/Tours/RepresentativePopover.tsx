...
export default function RepresentativePopover({ reps, canCreate, canEdit, canDestroy, onRepUpdated }: Props) {
    ...
    const handleCreate = (e: React.FormEvent) => {
        ...
        createForm.post(route('representatives.store'), {
            preserveScroll: true,
            onSuccess: () => {
                createForm.reset();
                onRepUpdated?.();
    ...
    const handleUpdate = (repId: number) => {
        editForm.put(route('representatives.update', repId), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('representatives.destroy', repToDelete.id), {
            preserveScroll: true,
    ...
    const handleRestore = (repId: number) => {
        router.post(
            route('representatives.restore', repId),
            {},
            {
                preserveScroll: true,
                onSuccess: () => onRepUpdated?.(),
                ...
                <Popover>
                    ...
                    <PopoverContent align="end" className="w-[420px] p-4">
                        <div className="space-y-4">
                            <div>
                                <h3 className="font-semibold">Manage Representatives</h3>
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will soft-delete {repToDelete?.name}. Existing tours already linked to this representative can still keep that link.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
